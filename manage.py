		...
		try:
			if form.validate() == False:
				...
				if not slug:
	   				...
	   				obj.update({"slug" : slugify(request.form['title']),"category_id":request.form['category_id'] , "title" : request.form['title'],'description':request.form['description'],'category_id':request.form['category_id'],'feature_image':filename })
	   				status = db.session.commit()
	   				if not status:
